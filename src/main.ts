...
function render(): void {
  ...
  const changed = dates.filter((date) => week.days[date]?.outcome === 'abandoned').length;
  ...
  app.innerHTML = `
    ...
    <header class="app-header">
      ...
      <div class="hero">
        <div class="hero-copy"><p class="eyebrow">Weekly dinner planner</p><h1>Plan dinners for the energy you have.</h1><p class="lede">For households balancing school meals, leftovers, and the cook’s changing energy.</p><div class="hero-action"><a class="primary-button" href="${isDemo ? '#planner' : '/demo/'}">${isDemo ? 'Explore this sample week' : 'Try it with sample data'}</a><span>${isDemo ? 'The planner is ready to use.' : 'See a planned week right away.'}</span></div><ul class="hero-facts"><li>Your plans stay in this browser.</li><li>Works offline after the first visit.</li><li>Free for 8 recipes. $12 once for more recipes and weeks.</li></ul></div>
        <figure class="hero-art"><picture><source srcset="/assets/week-rhythm.avif" type="image/avif"><source srcset="/assets/week-rhythm.webp" type="image/webp"><img src="/assets/week-rhythm.jpg" width="1200" height="800" alt="Seven geometric place settings arranged around two linked leftover bowls" fetchpriority="high" decoding="async"></picture><figcaption class="hero-caption">A week view with energy and leftover markers</figcaption></figure>
      </div>
    </header>
    <main id="main" class="app-main" tabindex="-1">
      ...
      <section id="planner" aria-labelledby="planner-title">
        <div class="section-heading"><div><p class="eyebrow">Weekly plan</p><h2 id="planner-title">Plan this week’s dinners</h2><p>Start with energy, then place meals. Warnings are prompts to reconsider—not nutrition or allergy advice.</p></div>
        ...
        <div class="plan-summary" aria-label="Week summary"><span class="summary-stat"><strong>${planned}</strong><span>nights planned</span></span><span class="summary-stat"><strong>${cooked}</strong><span>cooked</span></span><span class="summary-stat"><strong>${changed}</strong><span>changed</span></span><span class="summary-stat"><strong>${allWarnings.length}</strong><span>checks</span></span><span class="summary-stat"><strong>${groceries}</strong><span>grocery lines</span></span></div>
      </section>

      <div class="panel-grid">
        <section class="panel" id="recipes" aria-labelledby="recipes-title">
          <div class="panel-head"><div><p class="eyebrow">Your recipes</p><h2 id="recipes-title">Recipe cards</h2></div><button class="primary-button" type="button" data-action="add-recipe">+ Add recipe</button></div>
